    ...
    <div className="min-h-screen flex items-center justify-center p-4">
        ...
        <div className="mb-4">
            ...
            onComplete={async () => {
                ...
                // Si le joueur n'a pas encore voté, forcer un vote automatique ou passer au suivant
                if (!gameState.selectedAnswer) {
                console.log('⏰ Player did not vote, auto-advancing...');
                // Optionnel : soumettre un vote vide ou random
                // await actions.submitVote('NO_ANSWER');
                }
                
                // Note: Le serveur gère déjà l'auto-advance quand tous les joueurs ont voté
                // Mais si le timer expire, on peut forcer l'advance côté client
                // Pour l'instant, on laisse juste le message
                console.log('⏰ Waiting for server to advance to next round...');
            }} 
            />
        </div>
